...
RSpec.describe "Approval of Applications", type: :feature do
  ...
  it "Can't approve an application for an animal that is pending" do
    shelter_1 = Shelter.create( name:    "4 Paws Rescue",
                                address: "6567 W Long Dr.",
                                city:    "Littleton",
                                state:   "CO",
    ...
    pet_app1 = PetApplication.create(pet_id: pet_1.id, application_id: application1.id)
    pet_app2 = PetApplication.create(pet_id: pet_1.id, application_id: application2.id)
    ...
    within(".pet-app-#{pet_app2.id}") do
      expect(page).to_not have_link("Approve")
      expect(page).to have_content("Pending adoption by another applicant")
    end
  end
end